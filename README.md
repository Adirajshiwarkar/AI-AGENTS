...
cp .env.example .env
Fill in the configuration details. The system prefers Groq or Gemini Free but runs locally on Ollama or LM Studio seamlessly. If no LLM settings are supplied, the factory initializes the MockProvider which simulates the LLM outputs, enabling out-of-the-box pipeline validation without API keys.
...
Endpoint: POST http://localhost:8000/agent Header: Content-Type: application/json Body:
...
State Management: We chose an in-memory AgentState object. For persistent production use with millions of jobs, a Redis or PostgreSQL database state-machine (like Temporal or Celery) should track active agent sessions.
Synchronous Requests: The POST /agent runs synchronously. For large documents (taking >1 minute), a background task pattern returning a task_id with polling on GET /tasks/{id} is a better design. We chose synchronous execution here to match the strict single-request endpoint requirement.
Mock Provider fallback: Added to ensure the suite is instantly runnable and verifiable even without a Groq/Gemini key or a running Ollama container.
Future Improvements
